# Hi, I'm Varun M.N. 👋

### Hardware Design Engineer | Embedded Sysytem

I'm a Hardware Design Engineer with hands-on experience in PCB schematic design, PCB layout, hardware development, embedded software, debugging, testing, and validation.

I work with electronic circuits, embedded hardware, microcontrollers, and embedded software, with a focus on developing reliable and production-ready hardware and software solutions.

---

## 🛠️ Skills

### PCB Design

![OrCAD](https://img.shields.io/badge/OrCAD-Capture%20CIS-red)
![Cadence](https://img.shields.io/badge/Cadence-Allegro-blue)
![PCB Design](https://img.shields.io/badge/PCB-Design-green)
![Footprints](https://img.shields.io/badge/PCB-Footprints-yellow)

### Embedded Hardware & Software

![Embedded C](https://img.shields.io/badge/Embedded-C-blue)
![C](https://img.shields.io/badge/C-Programming-blue)
![Microcontrollers](https://img.shields.io/badge/Microcontrollers-green)
![Hardware Software](https://img.shields.io/badge/Hardware--Software-Integration-orange)

### Testing & Debugging

![Oscilloscope](https://img.shields.io/badge/Oscilloscope-red)
![Spectrum Analyzer](https://img.shields.io/badge/Spectrum%20Analyzer-purple)
![Signal Generator](https://img.shields.io/badge/Signal%20Generator-blue)
![Multimeter](https://img.shields.io/badge/Multimeter-green)
![Continuity Testing](https://img.shields.io/badge/Continuity%20Testing-orange)

### Hardware Development

![Hardware Bringup](https://img.shields.io/badge/Hardware-Bring--up-blue)
![Circuit Debugging](https://img.shields.io/badge/Circuit-Debugging-red)
![Hardware Validation](https://img.shields.io/badge/Hardware-Validation-green)
![RF Testing](https://img.shields.io/badge/RF-Testing-purple)
![Prototyping](https://img.shields.io/badge/Electronic-Prototyping-orange)

---

## 💼 Experience

### Associate Engineer – Hardware Design & Development

**Rangsons Aerospace Pvt. Ltd., Bengaluru**

**July 2026 – Present**

- PCB schematic design using OrCAD Capture CIS
- PCB layout and routing using Cadence Allegro
- Component footprint and library development
- Hardware bring-up and PCB debugging
- Hardware testing and validation
- RF amplifier board testing
- Embedded hardware and software development

---

## 🚀 Projects

### 🔹 RF Power Amplifier PCB Design & Validation

- Designed PCB schematics and layouts
- Performed PCB testing and hardware validation
- Conducted RF amplifier board testing
- Used spectrum analyzer and signal generator for measurements

### 🔹 Dynamic EV Wireless Charging System

- Developed a wireless power transfer prototype using Arduino
- Implemented resonant inductive coupling
- Designed transmitter and receiver coil arrangement
- Developed the prototype using Arduino C++

---

## 🔨 What I Build

- PCB designs and embedded hardware
- Embedded software and microcontroller systems
- Electronic circuits and prototypes
- RF amplifier boards
- Hardware bring-up and debugging
- Hardware testing and validation
- Wireless power transfer systems

---

## 🎓 Education

### B.E. – Electronics & Communication Engineering

**Cauvery Institute of Technology, Mandya**

2021 – 2025 | **CGPA: 8.79**

---

## 📫 Connect With Me

🌐 **Portfolio:**  
https://varun0531.github.io/

💼 **LinkedIn:**  
https://www.linkedin.com/in/varunmn0531/

📧 **Email:**  
mnvarun0531@gmail.com

📍 **Bengaluru, India**

---

⭐ **PCB Design • Embedded Systems • Hardware Development • Electronics • RF • Prototyping**

Thanks for visiting my profile! 🚀

